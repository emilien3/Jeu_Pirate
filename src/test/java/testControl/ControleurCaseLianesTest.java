package testControl;

import static org.junit.jupiter.api.Assertions.*;

import org.junit.jupiter.api.Test;

import boundary.BoundaryConsole;
import control.ControlJeuPirate;
import control.ControleurCaseLianes;
import control.ControleurCaseNourriture;
import model.De;
import model.Etat;
import model.Pirate;

class ControleurCaseLianesTest {
	De de1 = new De();
	De de2 = new De();
	De[] des = {de1,de2};
	BoundaryConsole sortie =  new BoundaryConsole();
	ControlJeuPirate cJP = new ControlJeuPirate(sortie, 2);
	ControleurCaseLianes cCL = new ControleurCaseLianes(cJP, sortie,des);
	Pirate[] pirates = cJP.getPirates();
	int tour = cJP.getJoueurCourant();
	
	@Test
	void actionTest() throws InterruptedException {
		Thread actionThread1 =  new Thread(() -> {
			cCL.action(pirates[tour]);
		});
		actionThread1.start();
        Thread.sleep(1000);
        actionThread1.stop();  
        actionThread1.join();
        assertEquals(pirates[tour].getEtat(),Etat.ESTPRISON);
	}
	
	@Test
	void finLancerEtatTest() throws InterruptedException {
		Thread actionThread =  new Thread(() -> {
			cCL.finLancer();
		});
		actionThread.start();
        Thread.sleep(1000);
        actionThread.stop();  
        actionThread.join();
	}
	
	boolean desValide(int n) {
		return n < 7 && n > 0;
	}
	
	@Test
	void getDesTest() throws InterruptedException {
		Thread actionThread =  new Thread(() -> {
			cCL.finLancer();
		});
		actionThread.start();
        Thread.sleep(1000);
        actionThread.stop();  
        actionThread.join();
        int[] newDes = cCL.getDes();
        assertEquals(desValide(newDes[0]),true);
        assertEquals(desValide(newDes[1]),true);
	}
	
}
